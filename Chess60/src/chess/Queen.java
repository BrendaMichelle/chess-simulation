...
public class Queen extends Piece {
	...
	public boolean checkValidMove(BoardCell[][] chessBoard, int new_x, int new_y) {
		...
		if(new_x - queenX == 0 && new_y - queenY == 0) {	// No movement
			return false;
		}
		
		if(new_x - queenX == 0) {						// Y is different
			verticalMovement = true;
		}
		else if(new_y - queenY == 0) {					// X is different
			horizontalMovement = true;
		}
		else if(Math.abs((new_y - queenY) / (new_x - queenX)) == 1) {		// Both X & Y are different
